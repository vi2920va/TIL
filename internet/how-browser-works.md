...
1. 받은 html 토큰은 **"html 이전"** 모드가 되고 토큰은 이 모드에서 처리된다. 이것은 HTMLHeadElement 요소를 생성하고 문서 객체의 최상단에 추가된다.
2. 상태는 **"head 이전"** 모드로 바뀌었고 **"body" 토큰을 받았다!** "head" 토큰이 없더라도 HtmlHeadElement 는 묵시적으로 생성되어 트리에 추가될 것이다. HTML의 관용?
